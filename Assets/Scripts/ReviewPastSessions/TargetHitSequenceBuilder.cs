...
public static class TargetHitSequenceBuilder
{
        ...
        double initialTime)
    {
        ...
        List<HitEvent> normalizedHits = recordedHits != null
            ? recordedHits.OrderBy(hit => hit.time).Select(hit => new HitEvent(hit.time, hit.location, hit.targetId)).ToList()
            : new List<HitEvent>();
        ...
        normalizedHits.Insert(0, new HitEvent(initialTime, firstConfiguredTarget, 1));
        return normalizedHits;
    }
